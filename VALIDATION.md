...
P2 probes and live run, same day, Codex CLI 0.155.1 then 0.156.0 (auto-updated), ChatGPT authentication:
- Under `-s read-only`, `cat` by absolute path and by `../` both read an outside secret, so Codex repo workers are refused.
- Even with `--ignore-user-config`, the default tool surface included GitHub write tools from the `apps` connectors, subagent tools and `view_image`. With only `shell_tool` disabled, `view_image` opened the outside secret, failing only at image decoding. Existing Codex review and inspection now disable those features.
- In 0.156.0, disabling `code_mode_host` also disabled web search; with it enabled (shell, file-viewing and connectors off), search worked, and code-mode `require`/`fetch` and `file://` page opens all failed to read the secret.
- Live P2 run: this Claude session as host, two Codex web workers on `gpt-6-sol`. Result `completed` (`cross_provider_panel`) in 50 seconds, with only `web_search` and model-message items; 4 of 7 claims were `retrieved` against the snippet of the cited URL. `0.156.0` is listed in `VALIDATED_CODEX_WEB_PANEL_CLI`.
- Hardened Codex review, live: a runner review on a disposable plan completed (`REVISE`, correct) with the connector features disabled. A tool listing under the same flags showed no GitHub tools, but user-configured MCP servers were still present. Per-server `-c mcp_servers.NAME.enabled=false` overrides now switch those off: a live runner review completed with `node_repl`, `playwright`, `tradingview` and `tradingview-desktop` recorded in `mcp_disabled`, and a tool listing under the recorded argv showed no MCP tools while shell access remained. Plugin-provided servers cannot be overridden with `-c` (the CLI rejects the entry as having no transport); they disappear with `--disable plugins`.
