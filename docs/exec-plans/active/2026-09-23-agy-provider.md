...
1. **Isolated profiles** (amended after the live canary; see below). `runner.py agy-profile` creates two owner-only profiles and prints a verified login command for each: `~/.claudex-loop/agy-web` for panel web workers (`toolPermission: request-review`, terminal sandbox, `allowNonWorkspaceAccess: false`, deny `write_file(*)`, `command(*)`, `unsandboxed(*)`, `mcp(*)`, `execute_url(*)`; allow `read_url(*)` and `read_file(<profile>/.gemini/antigravity-cli/brain)`) and `~/.claudex-loop/agy-review` for plan reviews (same, plus deny `read_file(*)` and `read_url(*)`; no allows). The login directory is created with the same empty-directory + ancestor-check routine used for launches (step 3) and rechecked before printing, because agy loads workspace customizations from its cwd and ancestors; the runner never performs or automates login. The user's own `~/.gemini` is never read or written.
2. **Preflight on every agy launch** (fail closed, no launch on any mismatch). Every agy process the runner starts — version, `mcp list`, `plugin list`, login probe, and the worker itself — runs with `HOME=<profile>` from a runner-created empty directory that passed the ancestor check in step 3; the runner's own cwd is never used:
   ...
   - `settings.json` keeps the role's security properties (agy rewrites the file, dropping default-valued keys such as `toolPermission` and adding `trustedWorkspaces`): preset absent or `request-review`, terminal sandbox on, no non-workspace access, every required deny present, allows exactly the role's list, no unknown keys;
...
3. **Invocation.** Environment is the parent environment (including `PATH`) with only `HOME` replaced by the role's profile. cwd is a fresh empty directory under the run artifacts, never the repo; no `--add-dir`. The runner walks from that cwd to `/` and refuses to launch if any `.agents`/`.agent`/`_agents`/`_agent`, `GEMINI.md` or `AGENTS.md` is found. Run directories therefore live under the system temp directory, not `$HOME`: this machine has `~/.agents/skills` (the Codex skills install target), which agy would load as workspace customizations.
