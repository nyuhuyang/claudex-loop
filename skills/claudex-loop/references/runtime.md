...
Run `python RUNNER agy-profile` once to create the locked `~/.claudex-loop/agy-home` profile and print a one-time login command. Login is manual. The runner never uses the user's `~/.gemini`. Every agy process uses that profile as `HOME`, a fresh empty cwd under system temp, and an ancestor check for workspace instructions. Preflight checks profile permissions/settings/configuration, `agy mcp list`, `agy plugin list`, login status and CLI version; settings and version are checked again immediately before each worker. The validated version is 1.2.9; an unvalidated version requires `--allow-unvalidated-cli` and is recorded. Windows agy runs are refused. agy runs require artifacts under system temp.

The agy invocation uses `--input-format stream-json --output-format stream-json --json-schema` and writes one user event to stdin. `--agy-model` defaults to `gemini-3.1-pro-high` and only `gemini-` slugs are accepted; `--agy-effort` is optional, and `--agy-cli` selects an absolute executable. The runner checks the requested `init.model`, but agy does not report the observed model. This transport and the profile's deny rules still need the live canary in [VALIDATION.md](../../../VALIDATION.md) before relying on agy. The terminal sandbox and permission rules are enforced by agy, leaving residual risk if its implementation changes.
...
- **agy workers:** web only, with no repository cwd or `--add-dir`. Stream tool events and the profile's full transcript must agree; only `search_web` and `read_url_content` are allowed. A missing transcript, disagreement or any other tool fails the panel without `panel.json`. A claim is `retrieved` only when its excerpt occurs in the `read_url_content` result for its exact URL; a search-only match is `unverified`. After each worker, the transcript is copied to owner-only artifacts and that conversation's brain, database files and summary row are removed from the profile. `--model`, `--effort`, `--cli` and spec model/effort apply only to the default opposite provider; agy uses `--agy-model`, `--agy-effort`, `--agy-cli`.
