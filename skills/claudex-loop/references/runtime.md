...
- Codex: `exec -s read-only`; resume uses `-c sandbox_mode="read-only"`. Review and inspection also disable the connector (`apps`), plugin, subagent, browser, image and dependency-install features that the installed CLI lists; the `apps` connectors otherwise expose GitHub write tools that the filesystem sandbox does not constrain. The result records `disabled_features`, and a failed feature probe stops the run. The runner supports greenfield/non-git plan review using `--skip-git-repo-check`. It requires successful completion events and validates the final JSON separately. MCP servers from the user's Codex configuration are still loaded (a `-c mcp_servers={}` override does not clear them, and `--ignore-user-config` would also drop the configured model); audit/disable write-capable integrations before review, because the shell sandbox is not a restriction on external MCP side effects. Never run the review with an unknown write-capable toolchain.
